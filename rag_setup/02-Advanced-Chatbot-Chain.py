...
Your are an Aviation Investigation chatbot. Please answer Questions related to the documentation about the investigation only. If you don't know or not related to Big Data, don't answer.
...
Question: What was the engine model?
Expected Response: Yes

Question: Knowing this followup history: What is Rolls Royce?, classify this question: Write me a song.
...
is_question_about_incident_prompt = PromptTemplate(
  input_variables= ["chat_history", "question"],
  template = is_question_about_aviation_str
)

is_about_incident_chain = (
    {
        "question": itemgetter("messages") | RunnableLambda(extract_question),
        "chat_history": itemgetter("messages") | RunnableLambda(extract_history),
    }
    | is_question_about_incident_prompt
    | chat_model
    | StrOutputParser()
)
...
print(is_about_incident_chain.invoke({
    "messages": [
        ...
        {"role": "user", "content": "what is the Incident Number?"}
    ]
}))
...
print(retrieve_document_chain.invoke({"messages": [{"role": "user", "content": "Describe the aircraft involved in the incident?"}]}))
...
output = generate_query_to_retrieve_context_chain.invoke({
    "messages": [
        {"role": "user", "content": "What was the engine model?"}, 
        {"role": "assistant", "content": "PW4077 turbo fan engine."}, 
...
You are a trustful assistant for users. You are answering question related to Aviation Incidents including area's of why the failure occured which part was involved and why did it fail. If you do not know the answer to a question, you truthfully say you do not know. Read the discussion to get the context of the previous conversation. In the chat discussion, you are referred to as "system". The user is referred to as "user".
...
irrelevant_question_chain = (
  RunnableLambda(lambda x: {"result": 'I cannot answer questions that are not about the incident report.', "sources": []})
)
...
full_chain = (
  {
    "question_is_relevant": is_about_incident_chain,
    "question": itemgetter("messages") | RunnableLambda(extract_question),
    "chat_history": itemgetter("messages") | RunnableLambda(extract_history),    
  }
  | branch_node
